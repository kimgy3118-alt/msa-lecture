    ...
    public void increaseEnrollmentCount(Long courseId) {
        try {
            webClientBuilder.build()
                    .post()
                    .uri(
                            "http://course-service/api/courses/internal/{id}/enrollment-count",
                            courseId
                    )
                    .retrieve()
                    .onStatus(
            ...
            log.info(
                    "[CourseServiceClient] 신청 인원 증가 완료 - courseId: {}",
                    courseId
            );

        } catch (IllegalStateException e) {
            // 접수 기간 종료 또는 정원 초과 오류를 Enrollment Service로 전달
            throw e;

        } catch (Exception e) {
            log.error(
                    "[CourseServiceClient] 신청 인원 증가 실패 - courseId: {}, error: {}",
                    courseId,
                    e.getMessage()
            );
